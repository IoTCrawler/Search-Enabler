...
public class AugmentedObservableProperty extends ObservableProperty {

    public AugmentedObservableProperty(String uri) {
        super(uri);
    }

    public AugmentedObservableProperty(ObservableProperty base) {
    ...
    public Object getIsObservedBy() {
        DataLoader loader = IoTCrawlerWiring.dataLoaderRegistry.getDataLoader(Sensor.class.getSimpleName());

        Object isObservedBy = isObservedBy();
        List ids = new ArrayList();
        if(isObservedBy instanceof Iterable)
            ids.addAll((List)isObservedBy);
        else
            ids.add(isObservedBy);
