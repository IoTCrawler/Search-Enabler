...
public class TestsSmartHome extends TestUtils {
    ...
    protected void initGraphQL() throws Exception {
        initGraphQL(Arrays.asList(Paths.get("schemas","smartHome.graphqls").toString()));
    }
    ...
    protected List<EntityLD> createEntities(){
        ...
        for(EntityLD entityLD: entities){
            try {
                Files.write(Paths.get("samples", "smartHome", Utils.getFragment(entityLD.getId().replace(":","-") + ".json")), Utils.prettyPrint(entityLD.toJsonObject()).getBytes());
            }
            catch (Exception e){
                e.printStackTrace();
            }
